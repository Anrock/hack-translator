...
parsing = describe "Assembly" $ describe "Parser" $ do
    ...
    describe "C instructions" $ do
        ...
        it "computation parts" $ do
            ...
            parse "D+A" `shouldBe` Right
                [Source $ CInstruction [] (Plus D (Reg A)) Nothing]
            parse "D+M" `shouldBe` Right
            ...
            parse "A-D" `shouldBe` Right
                [Source $ CInstruction [] (Minus A (Reg D)) Nothing]
            parse "M-D" `shouldBe` Right
...
processing :: SpecWith ()
processing = describe "Processing" $
  it "Symbol resolution" $
    resolve (Source <$> [ AInstruction (Label "Loc")  -- 0
