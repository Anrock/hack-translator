...
import Text.Megaparsec hiding (Label, label, parse)
...
aInstruction = P.label "A-Instruction" $
  lexeme $ do
    _ <- char '@'
    addr <- address <|> label
...
cInstruction = P.label "C-instruction" $
  lexeme $ do
    dest <- destination
    comp <- computation
    jmp <- optional jump
...
jump :: Parser Jump
jump =
  P.label "C-instruction jump part" $
    char ';'
      *> ( read
            <$> choice
              [ try $ symbol "JLT"
...
computation =
  P.label "C-instruction computation part" $
    try binary <|> unary
  where
    unary = unOp <*> operand
    binary = do
