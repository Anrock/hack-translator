...
data REPLCommand = ShowRamInterval Address Address
                 | ShowRam
                 | ShowListing
                 | Step
                 | LoadProgram String
...
clampRange (s', f') (s, f) | s' < s = clampRange (s, f') (s, f)
...
showBin w = "0b" <> replicate (16 - length significantStr) '0' <> significantStr
  where significantStr = showIntAtBase 2 intToDigit w ""
...
eval r@REPL{ram} (ShowRamInterval base offset) = do
  ...
  else do let ramValues = [ ram ! i | i <- indices ram, i >= base, i <= base + offset]
...
step r@REPL{rom, ram, cpu} = r{cpu = cpu', ram = ram'}
  where ram' = maybe ram (\v -> ram // [(fromIntegral a, v)]) outM
