...
decodeALUFlags i = let zx = bit 11
                       nx = bit 10
                       zy = bit 9
                       ny = bit 8
                       f  = bit 7
...
alu ALUFlags{..} x y = let proc n z v | z && n    = 0xFFFF
                                      | z         = 0x0
                                      | n         = complement v
                                      | otherwise = v
                           x' = proc nx zx x
...
step cpu@CPU { a, pc, d } ins m
  | ins `maskedBy` cType
    = cpu { a    = if writeA then out else a,
  ...
  | otherwise = cpu{a = ins, pc = pc + 1}
    where (flags, useM) = decodeALUFlags ins
